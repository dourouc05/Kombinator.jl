function solve(
    i::MinimumBudget{ElementaryPathInstance{T, Maximise}, T},
    ::DynamicProgramming,
) where {T}
...
function solve(
    ...
    for v in vertices(i.instance.graph)
        ...
        V[v, 0] =
            length(S[v, 0]) == 0 ? 0 :
            sum(i.instance.rewards[e] for e in S[v, 0])
    end
            ...
            for v in vertices(i.instance.graph)
                for w in inneighbors(i.instance.graph, v)
                    ...
                    if length(S[w, remaining_budget]) > 0
                        used_budget =
                            sum(i.weights[e] for e in S[w, remaining_budget])
                    end
                    ...
                    if V[w, remaining_budget] + i.instance.rewards[Edge(w, v)] >
                       V[v, β] && used_budget >= remaining_budget
                        changed = true

                        V[v, β] =
                            V[w, remaining_budget] +
                            i.instance.rewards[Edge(w, v)]
