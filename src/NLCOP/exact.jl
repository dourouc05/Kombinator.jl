struct ExactNonlinearSolver <: NonlinearCombinatorialAlgorithm
    solver::Any # MINLP solver with support for the nonlinear function.
end
...
function solve(
    ...
    @constraint(
        ...
        eq,
        dot_product == sum(
            nli.nonlinear_coefficients[i] * x[i] for
            i in eachindex(nli.nonlinear_coefficients)
        )
    )
    @constraint(
        m,
        cone,
        [1 + dot_product, 1 - dot_product, 2 * nonlinear_term] in
        SecondOrderCone()
    )
    ...
    @objective(
        ...
        Max,
        sum(
            nli.linear_coefficients[i] * x[i] for
            i in eachindex(nli.nonlinear_coefficients)
        ) + nonlinear_term
    )
    ...
    dict_sol =
        Dict(k => JuMP.value(x[k]) for k in eachindex(nli.linear_coefficients))
